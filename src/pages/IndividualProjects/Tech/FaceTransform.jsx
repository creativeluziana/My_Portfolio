...
import ProjectImage from '../../../assets/Projects/FaceTransform.png';
import ApproachImg from '../../../assets/Projects/FaceTransform.png';
import ResultsImg from '../../../assets/Projects/FaceTransform.png';
import { Link } from 'react-router-dom';
import Footer from '../../../components/Footer';

const FaceTransform = () => {
  const images = Array(6).fill({ src: ProjectImage, alt: 'Face Transform' });
  ...
  return (
    <ProjectLayout title="Face Transform" subtitle="Tech project" showHero={false} showBack={false} fullWidth={true} contentPadding="py-0" contentGap="space-y-0">
      <div className="relative isolate rounded-3xl p-4 sm:p-6 md:p-8">
        ...
        <section className="mx-auto mt-10 flex max-w-5xl flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <h1 className="text-5xl lg:text-6xl font-medium" style={{ background: 'linear-gradient(90deg, #FFFFFF 0%, #D770D7 50%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Face Transform</h1>
        ...
        <section className="mx-auto mt-3 max-w-5xl"><div className="flex flex-wrap gap-3">{tags.map((t) => (<span key={t} className="rounded-full border border-white/25 px-4 py-2 text-sm text-white/90">{t}</span>))}</div><p className="mt-4 max-w-3xl text-xl lg:text-2xl leading-relaxed text-gray-300" style={{ lineHeight: '1.6', wordSpacing: '0.05em', letterSpacing: '0.01em' }}>Case study content coming soon.</p></section>

        <section className="mx-auto mt-8 max-w-5xl"><h3 className="text-3xl lg:text-4xl font-normal text-white">Tech Stack</h3><div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{tech.map((t, i) => (<div key={`${t.label}-${i}`} className="flex items-center gap-4 rounded-xl border-2 border-white/50 bg-white/5 backdrop-blur-sm p-6 transition hover:border-fuchsia-400"><div className="flex h-14 w-14 items-center justify-center"><img src={t.icon} alt="" className="h-12 w-12 object-contain" loading="lazy" /></div><span className="text-lg lg:text-xl leading-relaxed font-normal text-white" style={{ lineHeight: '1.6', wordSpacing: '0.05em', letterSpacing: '0.01em' }}>{t.label}</span></div>))}</div></section>
